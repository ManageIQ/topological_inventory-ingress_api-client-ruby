  ...
  class ServiceOfferingAllOf
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TopologicalInventoryIngressApiClient::ServiceOfferingAllOf` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TopologicalInventoryIngressApiClient::ServiceOfferingAllOf`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'service_offering_icon')
    ...
    def hash
      [archived_at, description, display_name, distributor, documentation_url, extra, long_description, name, resource_timestamp, service_credential, service_inventory, service_offering_icon, source_created_at, source_deleted_at, source_ref, source_region, subscription, support_url].hash
    end
